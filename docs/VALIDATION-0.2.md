...
Version `0.2.0`, code `2`. This validation applies to the standalone Kotlin/Compose/Media3 application. Historical results for the first version are preserved in [VALIDATION.md](VALIDATION.md).
...
The release tooling checks for missing signing configuration or fallback signing, version/code increments, the exact phone+TV matrix, tag conflicts, checksums and artifact substitution. `actionlint` passed for both workflows, as did five regression tests for removing Android builds from the old `ottplay-foss`.
